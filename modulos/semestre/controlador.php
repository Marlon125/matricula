...
function controlador() {
    ...
    $semestreOBJ = new Semestre();
    $datos = getDatos();

    switch ($evento) {
        case vADMINISTRACION:
            $semestreOBJ->getSemestres();
            setSemestres($semestreOBJ->registros);
            ...
            break;
        case INSERTAR:
            $msg = $semestreOBJ->insertar($datos);
            header("location: administracion?msg=$msg");
            break;
        case vEDICION:
            $info = array();
            if (array_key_exists('idsemestre', $datos)) {
                $filtro = "WHERE semestre.idsemestre = " . $datos['idsemestre'] . " LIMIT 1";
                if ($semestreOBJ->getSemestres($filtro)) {
                    $info = $semestreOBJ->registros[0];
                    $info['error'] = 0;
                } else {
                    $info['error'] = 1;
                }
        ...
        case ELIMINAR:
            if (array_key_exists('idsemestre', $datos)) {
                $msg = $semestreOBJ->eliminar($datos);
                header("location: administracion?msg=$msg");
            } else {
                header("location: administracion?msg=0");
            }
...
function getDatos() {
    ...
    if ($_POST) {
        ...
        if (array_key_exists('msg', $_GET))
            $datos['msg'] = $_GET['msg'];
        if (array_key_exists('idsemestre', $_GET))
            $datos['idsemestre'] = $_GET['idsemestre'];
    }
    return $datos;
}
