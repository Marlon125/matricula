...
function controlador() {
    ...
    $programaOBJ = new Matricula();
    $datos = getDatos();

    switch ($evento) {
        case vADMINISTRACION:
            $programaOBJ->getProgramas();
            setProgramas($programaOBJ->registros);
            ...
            break;
        case INSERTAR:
            $msg = $programaOBJ->insertar($datos);
        ...
        case vEDICION:
            ...
            if (array_key_exists('idprograma', $datos)) {
                ...
                if ($programaOBJ->getProgramas($filtro)) {
                    $info = $programaOBJ->registros[0];
                    $info['error'] = 0;
                } else {
                    $info['error'] = 1;
                }
        ...
        case ELIMINAR:
            if (array_key_exists('idprograma', $datos)) {
                $msg = $programaOBJ->eliminar($datos);
                header("location: administracion?msg=$msg");
            } else {
                header("location: administracion?msg=0");
            }
